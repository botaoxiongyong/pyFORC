...
class dataLoad_in(object):
    ...
    def matrix(self):
        ...
        xi,yi = np.mgrid[np.min(self.x):np.max(self.x):100j,np.min(self.y):np.max(self.y):100j]
        ...
        print(zi[1,:][0:2])
        print(np.count_nonzero(~np.isnan(zi[1,:])))
        print(self.y_range[0:2])

        f = np.polyfit(self.y_range[0:2],zi[1,:][0:2],1)
        fx = np.poly1d(f)

        print(fx(self.y_range[2]))
        ...
        for m in np.arange(1,len(xi)-1):
            ...
            f = np.polyfit(self.y_range[0:ynum],zi[m,:][0:ynum],3)
            fx = np.poly1d(f)
            for j in np.arange(0,10,step=1):
